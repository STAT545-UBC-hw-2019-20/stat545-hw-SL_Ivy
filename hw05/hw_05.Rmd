...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_reorder(continent, pop, max), fill=continent)) +
  coord_flip()+   # flip x and y
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")+ggtitle('Reordered by the max pop')
```
```{r}
gapminder %>%
  ggplot() +
  ...
  coord_flip()+   # flip x and y
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")+ggtitle('Original order')
...
```{r, message = FALSE, warning=FALSE}
ggsave("new_figure.jpg", width = 16, height =14, units = "cm")
```
